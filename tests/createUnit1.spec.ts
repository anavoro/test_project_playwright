...
test.describe("Test create_unit page 1", async () => {
    ...
    test('C367 Verify image upload panels', async ( {page} ) => {
        ...
        await expect(createAnnouncementPage.clueLine).toBeVisible();
        await expect(createAnnouncementPage.clueLine).toHaveText(/від 1 до/, /фото/);
    ...
    test('С384 Verify same images uploading', async ( {page} ) => {
        allImagesBlocks = await createAnnouncementPage.imageBlock.all()
       for ( let i = 0; i < allImagesBlocks.length - 2; i++) {
        ...
        await fileChooserFunc(allImagesBlocks[1], images[0], createAnnouncementPage, page)
    ...
    test('C401 Verify uploading of invalid file type', async ( {page} ) => {
        await fileChooserFunc(createAnnouncementPage.imageUnit, invalidData.fileType, createAnnouncementPage, page)
    ...
    test('C405 Verify uploading of invalid size file', async ( {page} ) => {
        await fileChooserFunc(createAnnouncementPage.imageUnit, invalidData.fileSize, createAnnouncementPage, page)
        await createAnnouncementPage.imageUnit.focus()
        await expect(createAnnouncementPage.popUpInvalidImageError).toHaveText(/не підтримується/);
        await createAnnouncementPage.clickPopUpInvalidImageCloseIcon();
        await expect(createAnnouncementPage.popUpInvalidImage).not.toBeVisible()
    ...
    test('C390 Verify Prev button', async ( {page} ) => {
    ...
    test('C393 Verify Next button', async ( {page} ) => {
        ...
        await createAnnouncementPage.clickNextButton();
        await expect(createAnnouncementPage.clueLine).toHaveCSS('color', color.error)
    ...
    test('C409 Verify input section and choosing of existing sevice', async ( {page} ) => {
        await fileChooserFunc(createAnnouncementPage.imageUnit, images[0], createAnnouncementPage, page)
        await createAnnouncementPage.clickNextButton();
        ...
        await expect(createAnnouncementPage.servicesSearchInput).toHaveAttribute('placeholder', /Наприклад/)
        ...
        await createAnnouncementPage.copyPaste(createAnnouncementPage.servicesSearchInput, invalidData.text101);
        await expect(createAnnouncementPage.symbolsCounter).toContainText(`${invalidData.text101.slice(0,-1).length} / 100`);
        ...
        const minSearchSymbolsAmount = await createAnnouncementPage.servicesSearchInput.getAttribute('value')
        ...
        const searchResultFirstLine = await searchResult[0].textContent()
        ...
        await expect((searchResultFirstLine.toUpperCase()).includes(searchService.toUpperCase())).toBeTruthy()
    ...
    test('C411 Verify choosing multiple services', async ( {page} ) => {
        ...
        for (let i = 0; i < searchResults.length; i++){
            expect((searchResults[i].toUpperCase()).includes(validData.letter)).toBeTruthy()
        }
